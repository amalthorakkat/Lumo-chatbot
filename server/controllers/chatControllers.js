...
const sendMessage = async (req, res) => {
  ...
  try {
    ...
    console.log("Session retrieved:", session ? session._id : "Not found");
    if (!session) {
      console.error(`Session not found: ${sessionId}`);
      return res.status(404).json({ message: "Session not found" });
    }
    ...
    console.error("Database Error (Session Retrieval):", {
      error: error.message,
      stack: error.stack,
    });
  ...
  try {
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
    console.log("Gemini API model initialized");
    const history = session.messages.slice(0, -1).map((msg) => ({
      role: msg.sender === "user" ? "user" : "model",
      parts: [{ text: msg.text }],
    }));
    ...
    try {
      ...
      console.error(
        "Database Error (Saving after Gemini failure):",
        saveError.message
      );
      return res.status(500).json({
        message: "Failed to save session after AI error",
        error: saveError.message,
      });
    }
  }

  // Add the bot's reply and update session title
  try {
    session.messages.push({ sender: "bot", text: reply });
    if (session.messages.length === 2) {
      session.title =
