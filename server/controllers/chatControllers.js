  ...
  try {
    ...
    console.log(
      `Session query result: ${
        session
          ? JSON.stringify({ id: session._id, userId: session.userId.toString() })
          : "null"
      }`
    );
    ...
    if (session.userId.toString() !== req.user.userId) {
      console.error(
        `Unauthorized: Session userId ${session.userId.toString()} does not match req.user.userId ${req.user.userId}`
      );
      return res
        .status(403)
